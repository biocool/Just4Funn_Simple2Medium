...
def getMilestoneDays(revenues, milestones):
  ...
  n_of_revenues = len(revenues)
  n_of_milestones = len(milestones)
  #revenues_sorted = revenues.sort()
  #milestones_sorted = milestones.sort()
  revenues_cuml = revenues.copy()
  for i in range((n_of_revenues - 1) ):
    revenues_cuml[i + 1] += revenues_cuml[i]

  
  mileStoneDaysArr = [-1 for i in range(n_of_milestones)]
  mileStoneDaysArr_index = 0 
  for current_milestone in milestones:
    revenues_cuml_index = 0
    while (revenues_cuml[revenues_cuml_index] < current_milestone) and (revenues_cuml_index < n_of_revenues-1):
      revenues_cuml_index += 1
    if revenues_cuml[revenues_cuml_index] >= current_milestone:
      mileStoneDaysArr[mileStoneDaysArr_index] = revenues_cuml_index + 1
      mileStoneDaysArr_index += 1       
  return mileStoneDaysArr
